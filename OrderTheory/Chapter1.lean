...
--attribute [local inline] WithTop.map

#check WithTop.orderIsoSumLexPUnit
/- def Ch_1_31 [PartialOrder P] : (LowerSet P)ᵒᵈ ≃o LowerSet (Pᵒᵈ) := by 
  sorry  -/
example [PartialOrder P] [PartialOrder Q] (f : P ≃o Q) : WithTop P ≃o WithTop Q := 
 {
  ...
  map_rel_iff' := @fun a b => by 
    ...
    · simp [WithTop.map]; 
      ...
      · rw [WithTop.none_eq_top, WithTop.some_eq_coe] at le
        exfalso; exact WithTop.not_top_le_coe ↑(f b) le 
      · rw [WithTop.none_eq_top, WithTop.some_eq_coe] at le
        exfalso; exact WithTop.not_top_le_coe ↑b le 
    · simp [WithTop.le_none] 
    · simp 
 }
  /- ⟨--Equiv.optionCongr f, 
   Option.map f.toFun, f.invFun, 
   @fun a b => by
   simp only [Equiv.optionCongr, EquivLike.coe_coe] 
   ...
   | some a => cases b with 
     ...
     | none => 
       simp [WithTop.le_none] 
       apply WithTop.le_none 
   | none => cases b with 
     | some b => 
       constructor <;> intro le
       · exfalso; rw [WithTop.none_eq_top] at le 
         apply f.map_rel_iff'.1 at le 
         apply WithTop.not_top_le_coe b le 
     | none => simp 
   · simp 
   · simp 
   · simp [WithTop.le_none] 
